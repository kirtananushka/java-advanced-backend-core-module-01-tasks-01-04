...
public class DeadlockDemoLogger {
   ...
   private static void logWithIcon(String threadName, String icon, String message, boolean isError) {
      ...
      System.out.printf("%s[%s]%s %-20s %-2s %s%n",
            ...
            String.format("%s[%s]%s", color, padRight(threadName, 15), ANSI_RESET),
            icon,
      ...
      String logLineHtml = String.format(
            "<pre style=\"margin:0;padding:0;color:%s;\">[%s] [<span style=\"display:inline-block;width:15ch;color:%s;\">%s</span>] <span style=\"color:%s;\">%s %s</span></pre>",
            HTML_CYAN,
            timestamp,
            htmlColor,
      ...
      String logLineText = String.format(
            "[%s] [%s] %s %s",
            timestamp,
            padRight(threadName, 15),
            icon,
            message
      );
      writeToTextFile(logLineText);
   }
   ...
   private static void writeToTextFile(String logLine) {
   ...
   private static String getString(String threadName, String ansiPurple, String ansiGreen, String ansiYellow, String ansiBlue, String ansiRed, String ansiReset) {
      if (threadName.equals("Main")) {
         return ansiPurple;
      } else if (threadName.contains("Writer")) {
         return ansiGreen;
      } else if (threadName.contains("Sum")) {
         return ansiYellow;
      } else if (threadName.contains("Sqrt")) {
         return ansiBlue;
      } else if (threadName.contains("Unsafe")) {
         return ansiRed;
      }
      return ansiReset;
   }
   ...
   public static void logSeparator() {
      ...
      writeToTextFile(separator);
   }

   public static void closeHtmlLogFile() {
      try (BufferedWriter writer = new BufferedWriter(new FileWriter(LOG_FILE + ".md", true))) {
         writer.write("");
      } catch (IOException e) {
